...
int main( int argc, const char * argv[] )
{
	...
	string first ="";
	first+=argv[1];
	int lenOfWord = strToInt(first.data());
	//
    ...
    int width = lenOfLetter*lenOfWord; 
    int height = 64; 
	...
	for(int i=0;i<lenOfWord;i++){
		text[i] = rand()%26;//TODO: normal const
	}

	for(int c=0;c<lenOfWord;c++){
			
		for(register int x=lenOfLetter*c;x<lenOfLetter*(c+1);x++){
			for(register int y=0;y<height;y++){
					
				int pos =  (x + y * width);
					
				byte color = alphabet.alphabet[text[c]][x%lenOfLetter+y*lenOfLetter];
	...
	byte newData[width*height*3] = {bgc};
	{
	//period
    ...
    for(register int x=0;x<width;x++){
        for(register int y=0;y<height;y++){
            byte color_diff = abs(bgc - imageData[x+y*width]);
            if(!color_diff){
                continue;
            }
            //source x (double)
